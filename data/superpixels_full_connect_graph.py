...
def compute_adjacency_matrix_images(coord, feat, use_feat=True, kth=8):
    ...
    d = cdist(coord,coord, metric='euclidean')
    e = cdist(coord, coord, lambda u, v: np.sqrt(((u-v)**2).sum()))
    f = cdist(coord, coord, lambda u, v: np.abs(u-v).sum())
...
#    c_dist = cdist(coord, coord, metric = 'mahalanobis') #############
#    c_dist = cdist(coord, coord, metric = 'correlation') #############
    c_dist = cdist(coord, coord, lambda u, v: np.abs(u-v).sum())
    
    if use_feat:
        # Compute feature distance
        
#        f_dist = cdist(feat, feat)
#        f_dist = cdist(feat, feat, metric = 'mahalanobis') #############
#        f_dist = cdist(feat, feat, metric = 'correlation') #############
        f_dist = cdist(feat, feat, lambda u, v: np.abs(u-v).sum())
...
class SuperPixDGL(torch.utils.data.Dataset):
    ...
    def _prepare(self):
        ...
        for index, sample in enumerate(self.sp_data):
            ...
            #print('A',A.shape,A)
            #print('edge_values_list',len(edge_values_list),edge_values_list[0])
...
class SuperPixDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name):
        ...
        pklname = data_dir+name+'.pkl'   ########################
        #pklname = data_dir+name+'_mahalanobis'+'.pkl'
        #pklname = data_dir+name+'_correlation'+'.pkl'
        #pklname = data_dir+name+'_l1'+'.pkl'
        
        print('=> load pickle: ', pklname)
...
#            graphs[idx].edata['feat'] = graph.edata['feat'].float()
            #print('ndata',graphs[idx].ndata['feat'].shape,type(graphs[idx].ndata['feat']),graphs[idx].ndata['feat'].dtype)
            #print('edata',graphs[idx].edata['feat'].shape,type(graphs[idx].edata['feat']),graphs[idx].ndata['feat'].dtype)
